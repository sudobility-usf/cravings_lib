...
**npm**: `@sudobility/cravings_lib` (restricted, BUSL-1.1)
...
Unified hook that combines cravings_client hooks + Zustand store + business logic:
...
This is the primary hook consumed by UI layers (cravings_app, cravings_app_rn).
...
```
cravings_app / cravings_app_rn
    ↓ uses
@sudobility/cravings_lib (this package)
    ↓ uses
@sudobility/cravings_client (API hooks)
    ↓ uses
@sudobility/cravings_types (type definitions)
```

## Related Projects

- **cravings_types** — Shared type definitions; imported transitively via cravings_client
- **cravings_client** — API client SDK; this library wraps its hooks with business logic and Zustand state
- **cravings_app** — Web frontend that consumes `useHistoriesManager` from this library
- **cravings_app_rn** — React Native app that consumes `useHistoriesManager` via file: links
- **cravings_api** — Backend server; this library communicates with it indirectly through cravings_client

## Coding Patterns

- `useHistoriesManager` is the primary hook -- it orchestrates cravings_client hooks + Zustand store into a single unified interface for UI layers
